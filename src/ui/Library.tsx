...
export function Library({
   ...
   * soit le rayon d'où l'on venait. Quand `rayon` est fourni, c'est le
   * parent qui se souvient — et il survit au démontage.
   *
   * Les deux props sont optionnelles : sans elles, l'écran garde son état
   * local et se comporte comme avant.
   */
  rayon?: Tab;
  onRayon?: (t: Tab) => void;
  ...
  /*
   * Le rayon vient du parent s'il le tient, sinon de l'état local. Les
   * quatre `setTab` de cet écran passent par le même chemin : ils
   * écrivent les deux, donc le comportement est identique dans les deux
   * montages.
   */
  const [tabLocal, setTabLocal] = useState<Tab>('travail');
  const tab = rayon ?? tabLocal;
  const setTab = (t: Tab) => { setTabLocal(t); onRayon?.(t); };
